//********************************************************************************************
//
// Bumpmapping with 8 lights ( 4 passes) (ps v1.4 , vs v1.1)          
// by EVOLVED 	   	   
//
//	
/////////////////////////////////////////////
...
//********************************************************************************************
// Textures
   texture colorTexture < string ResourceName = "<defalt>"; >;	// grab our base texture (TextCord 0)
	sampler2D colorSampler = sampler_state // grab our base sampler
		{
		Texture = <colorTexture>;
		};
   texture normalTexture <string ResourceName = "<defalt>"; >; // grab our noramlmap texture (TextCord 1)
...
VSOutput mainVS1(appdata IN) // 1st vertex data for light 1-4
	...
	float3 v = ViewInv[3].xyz - WP ; 
	OUT.WorldEyeVec = normalize(mul(v,TM));
  return OUT;
}
...
float4 PS_bumpmap_lights5(VSOutput IN)  : COLOR
{
	...
	float4 TB=tex2D(colorSampler,IN.uv);
   	float4 NB=tex2D(normalSampler,IN.uv)*2-1;
	 	float3 LV=IN.Light1;
	 		float GLB=saturate(dot(NB,LV));
			GLB = GLB + pow(saturate(dot(NB,LV)),1);
	 			float4 TN=GLB*LC_5 ;
		float4 result= TB * TN ;
   	return result;
}
	...
	technique lighting1
 		{
			pass one
				{		
					VertexShader = compile vs_1_1 mainVS1(); // VS 1.1
					PixelShader = compile ps_1_4 PS_bumpmap_lights1(); // PS 1.4				
				}
			}
	technique lighting2
 		{
